#include "flock.hpp"
//accumulate accetta anche come 4 parametro una tua funzione che è ciò che fa invece di sommare gli elementi del range
...
// globale è stata considerata come la media delle distanze medie dei singoli
...
double Flock::mean_distance() const
{
  ...
  for (Boid const& fixed_boid : flock) {//è un range for

    double const single_sum{
      
      std::accumulate( flock.begin(), flock.end(), 0., [&](double sum, Boid const& boid) { //la reference nella lambda serve per prendere fixed_boid //sum deve essere nell'input della lambda perchè viene inizializzata con il valore
          return sum + std::sqrt((fixed_boid.get_x() - boid.get_x()) 
    ...
    total_sum += single_sum / (flock.size() - 1);//total sum aggiornato a ogni iterazione del for
    assert(total_sum >= 0);
...
double Flock::mean_velocity() const
{
  if (flock.size() == 0)
    return 0.;
  double const total_sum{std::accumulate(flock.begin(), flock.end(), 0., [&](double sum, Boid const& boid) {return sum+ std::sqrt(boid.get_vx() * boid.get_vx()+ boid.get_vy() * boid.get_vy());})
    };
...
double Flock::stnd_deviation_distance(double mean_distance) const
{
  ...
  for (Boid const& fixed_boid : flock) {
    double const single_sum{std::accumulate(flock.begin(), flock.end(), 0., [&](double sum, Boid const& boid) {
          return sum
...
double Flock::stnd_deviation_velocity(double mean_velocity) const
{
  if (flock.size() == 1 || flock.size() == 0)
    return 0;
  double const total_sum{std::accumulate(
      flock.begin(), flock.end(), 0., [&](double sum, Boid const& boid) {return sum+ (std::sqrt(boid.get_vx() * boid.get_vx()+ boid.get_vy() * boid.get_vy())- mean_velocity)* (std::sqrt(boid.get_vx() * boid.get_vx()+ boid.get_vy() * boid.get_vy())- mean_velocity);})
      }; //sum all'inizio è zero ma man mano che accumulate scorre il range gli vengono sommate varie cose
  assert(total_sum >= 0);
  assert(flock.size() != 0 && flock.size() != 1);
  return std::sqrt(total_sum / (flock.size() - 1));
}
