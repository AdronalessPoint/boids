...
int input_reader()
{
  std::string input{std::to_string(30)};//stringa dove verrà salvato il valore in input che dai
  try {
    std::cout << "Inserire il numero di Boid da generare:" << '\n';


    //std::getline(std::cin, input); //ho commentato e messo 30 il valore di default della stringa di imput, così non lo chiede ogni volta
    
    
    if (is_integer(input)) { //controlla che ciò che è nella stringa sia un intero (no virgole o punti o lettere ma accetta i numeri negativi(interi))
      int const value{std::stoi(input)};//estrae un intero dalla stringa
      if (value > 0) {
  ...
  return input_reader(); //se sbagli ti gestisce le eccezioni e ti richiede tutto da capo (ricorsivo)
  }
}
...
int main()
{
  ...
  Flock stormo{5., 5., 5., n}; //valori iniziali di sep  alignement e cohesion
  assert(n > 0);
  stormo.fill();
  assert(!stormo.get_flock().empty()); //controlla stormo non sia vuoto
  graphics(stormo);
