...
// usare fabs invece di abs è da terroristi, non cambia un cazzo
// overload di range nel caso in cui si utilizzi per l'interazione di coesione e
// allineamento
bool in_range(double range, Boid const& boid, Boid const& fixed_boid) {
  assert(range > 0);
  return std::abs(boid.get_x() - fixed_boid.get_x()) < range &&
         std::abs(boid.get_y() - fixed_boid.get_y()) < range &&
         !(boid == fixed_boid);
}
...
// Viene definito il corpo del metodo che calcola la velocità repulsiva lungo le
// X. La funzione è chiamata su di un singolo fixed_boid che le viene passato,
// in questo modo andrà a scorrere sul vettore dei Boid calcolando l'interazione
...
double Flock::vx_separation(double range, Boid const& fixed_boid) const {
  double sum{0.};
  for (Boid const& boid : flock) {
    if (in_range(range, boid, fixed_boid) && !boid.get_is_dead()) {
  ...
  double const vx_rep{-sum * sep_};  // il meno è perchè vuoi vada via
  // Se il valore ottenuto è più basso di quello massimo, allora può essere
  // returnato...
  if (std::abs(vx_rep) < max_v_rep) {
    return vx_rep;
  }
  // ...altrimenti calcola il verso della velocità e returna l'intensità massima
  // (se entra nell'if la funzione finisce perchè c'è il return che restituisce
  // il ocntrollo al chiamante)
  double const sign{std::abs(vx_rep) / vx_rep};
...
double Flock::vy_separation(double range, Boid const& fixed_boid) const {
  double sum{0.};
  for (Boid const& boid : flock) {
    if (in_range(range, boid, fixed_boid) && !boid.get_is_dead()) {
  ...
  if (std::abs(vy_rep) < max_v_rep) {
    return vy_rep;
  }
  double const sign{std::abs(vy_rep) / vy_rep};
  return sep_ * max_v_rep * sign;
}
...
double Flock::vx_alignment(double range, Boid const& fixed_boid) const {
  //  count_if() function returns the number of elements in a range that satisfy
  //  the condition.
  long int const n_boids{
      std::count_if(flock.begin(), flock.end(), [&](Boid const& boid) {
        return in_range(
            range, boid,
            fixed_boid);  // la lambda è un predicato , ritorna vero ofalso
      })};
  if (n_boids == 0) {
    return 0.;
  }
  ...
  if (std::abs(vx_align) < max_v_alig) {
    return vx_align;
  }
  double const sign{std::abs(vx_align) / vx_align};
  return al_ * max_v_alig * sign;
}
// Anche in questo caso, come lo sarà per tutte le funzioni, la gemella sulle y
// è identica
double Flock::vy_alignment(double range, Boid const& fixed_boid) const {
  long int const n_boids{std::count_if(
      flock.begin(), flock.end(),
      [&](Boid const& boid) { return in_range(range, boid, fixed_boid); })};
  if (n_boids == 0) {
    return 0.;
  }
  assert(n_boids > 0);
  ...
  if (std::abs(vy_align) < max_v_alig) {
    return vy_align;
  }
  double const sign{std::abs(vy_align) / vy_align};
  return al_ * max_v_alig * sign;
}
...
double Flock::vx_cohesion(double range, Boid const& fixed_boid) const {
  long int const n_boids{std::count_if(
      flock.begin(), flock.end(),
      [&](Boid const& boid) { return in_range(range, boid, fixed_boid); })};
  if (n_boids == 0) {
  ...
  assert(coh_ >= 0 || coh_ <= 10);
  double const vx_cohe{coh_ * (sum / n_boids - fixed_boid.get_x()) / 3.};
  if (std::abs(vx_cohe) < max_v_cohe) {
    return vx_cohe;
  }
  double const sign{std::abs(vx_cohe) / vx_cohe};
  return coh_ * max_v_cohe * sign;
}
// Funzione gemella per le y
double Flock::vy_cohesion(double range, Boid const& fixed_boid) const {
  long int const n_boids{std::count_if(
      flock.begin(), flock.end(),
      [&](Boid const& boid) { return in_range(range, boid, fixed_boid); })};
  if (n_boids == 0) {
  ...
  assert(coh_ >= 0 || coh_ <= 10);
  double const vy_cohe{coh_ * (sum / n_boids - fixed_boid.get_y()) / 3.};
  if (std::abs(vy_cohe) < max_v_cohe) {
    return vy_cohe;
  }
  double const sign{std::abs(vy_cohe) / vy_cohe};
  return coh_ * max_v_cohe * sign;
}
// NOTA FINALE
...
// Definizione di due variabili che contengono il valore del centro del
// rettangolo in cui si trovano i boids. Queste serviranno poco dopo per il
// calcolo della velocità di rientro.
...
// contemporanemente, no sta puntando in fuoribensì sta rientrando. Questo
// secondo caso è responsabile di mantenere lo stormo vivo e reattivo, facendolo
// accelerare verso il centro qualora provasse ad evadere dal rettangolo. La
// presenza di primo e secondo limite garantisce così una specie di pista di
// accelerazione per i boid, senza però rischiare che vengano accelerati verso
// l'esterno. Chiaramente esiste la gemella sulle y di questa funzione.
bool p_velx_active(Boid const& boid) {
  if (boid.get_x() < bound_xmin) {
    bool const is_center_oriented{boid.get_vx() >
                                  0.};  // non sta puntando verso fuori
...
// Definizione della funzione che calcola la velocità di rientro del boid,
// questa è direttamente proporzionale alla distanza ed è regolata da una
// distanza moltiplicativa.
...
bool in_explosion_range(double expl_centerx, double expl_centery,
  ...
  assert(expl_range > 0);
  return (std::abs(boid.get_x() - expl_centerx) < expl_range &&
          std::abs(boid.get_y() - expl_centery) < expl_range);
}
...
double expl_velocity_x(double b, double expl_centerx, Boid const& boid) {
  ...
  if (std::abs(expl_velx) < max_expl_vel) return expl_velx;
  double const sign{std::abs(expl_velx) / expl_velx};
...
double expl_velocity_y(double b, double expl_centery, Boid const& boid) {
  ...
  if (std::abs(expl_vely) < max_expl_vel) return expl_vely;
  double const sign{std::abs(expl_vely) / expl_vely};
  return max_expl_vel * sign;
}
