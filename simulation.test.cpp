...
SUBCASE("Test della funzione random per le velocità iniziali") {
  ...
  bool const check1{std::abs(a) <= initial_vel &&
                    std::abs(a) >= (initial_vel - 500.)};
  CHECK(check1);
}
}
;
...
TEST_CASE("Test sulle funzioni dello stormo") {
  ...
  SUBCASE("Test dei metodi get per i parametri") {
    ...
    CHECK(stormo.get_al() == 5);
    CHECK(stormo.get_coh() == 5);
  }
  SUBCASE("Test dei metodi set per i parametri") {
    ...
    CHECK(stormo.get_al() == 7);
    CHECK(stormo.get_coh() == 8);
  }
...
TEST_CASE("Test per i metodi di calcolo per le velocità d'interazione") {
  ...
  SUBCASE("Test per la v di separazione") {
    ...
    // quando no
    CHECK(stormo.vx_separation(100, stormo.get_flock()[0]) < 0);
    CHECK(stormo.vx_separation(5, stormo.get_flock()[0]) == 0);
    CHECK(stormo.vy_separation(100, stormo.get_flock()[0]) < 0);
    CHECK(stormo.vy_separation(5, stormo.get_flock()[0]) == 0);
  }
  SUBCASE("Test per la v di allineamento") {
    // Controllo che sia assente se eccede il range massimo d'attivazione
    CHECK(stormo.vx_alignment(8, stormo.get_flock()[0]) == 0);
    // Controllo sia presente e nel verso giusto quando in range
    CHECK(stormo.vx_alignment(100, stormo.get_flock()[0]) > 0);
   
    // Controllo che sia assente se eccede il range massimo d'attivazione
    CHECK(stormo.vy_alignment(8, stormo.get_flock()[0]) == 0);
    // Controllo sia presente e nel verso giusto quando in range
    CHECK(stormo.vy_alignment(100, stormo.get_flock()[0]) < 0);
  }
  SUBCASE("Test per la v di coesione") {
    
    // Controllo che sia assente se eccede il range massimo d'attivazione
    CHECK(stormo.vx_cohesion(5, stormo.get_flock()[0]) == 0);
    // Controllo sia presente e nel verso giusto quando in range
    CHECK(stormo.vx_cohesion(50, stormo.get_flock()[0]) > 0);
   
    // Controllo che sia assente se eccede il range massimo d'attivazione
    CHECK(stormo.vy_cohesion(5, stormo.get_flock()[0]) == 0);
    // Controllo sia presente e nel verso giusto quando in range
    CHECK(stormo.vy_cohesion(50, stormo.get_flock()[0]) > 0);
  }
};
...
TEST_CASE("Test per le free-function definite in simulation.cpp") {
  ...
  SUBCASE("Test nel caso di uno stormo non statico") {
    ...
    update(stormo, 1, delta_t);
    for (int i{0}; i < stormo.get_size(); ++i)
...
TEST_CASE("Test per le funzioni di modifica degli oggetti grafici") {
  ...
  sf::RectangleShape rectangle{make_rectangle( rectangle_width, rectangle_height, -5.f, color,
  ...
  SUBCASE("Test per il controllo dell'origine") {
    CHECK(rectangle.getOrigin() == sf::Vector2f(0, rectangle_height));
  }
  SUBCASE("Test per il controllo della posizione") {
    CHECK(rectangle.getPosition() == sf::Vector2f(0, display_height));
  }
  rectangle.setOrigin(rectangle_width, rectangle_height);
  SUBCASE("Test per il controllo dell'origine") {
    CHECK(rectangle.getOrigin() == sf::Vector2f(rectangle_width, rectangle_height));
  }
  rectangle.setOrigin(rectangle_width/2, rectangle_height/2);
  SUBCASE("Test per il controllo dell'origine") {
    CHECK(rectangle.getOrigin() == sf::Vector2f(rectangle_width/2, rectangle_height/2));
  }
  

  sf::Font font;
  font.loadFromFile("./boid_utilities/fonts/arial.ttf");
  sf::Text text{make_text(font, 16, 1.f, "Testo", sf::Color::Black,
