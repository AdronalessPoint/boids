...
std::string roundto(double const num, int const n) {
  std::string string{std::to_string(num)}; //stringa fatta a partire da un double
  auto s = std::find(string.begin(), string.end(), '.'); //trova la posizione del carattere punto

  if(s + n < string.end()){//se approssimazione scelta è troppo blanda non fa nulla
      string.erase(s + n, string.end()); //se approssimazione scelta è significativa cancella ciò che non serve
    }
    return string;
}
...
// vettore(della std library) di vettori(di sfml) in pratica i punti descritti dai
...
sf::ConvexShape Button::getShape() const { return shape_; } //draw è un metodo di Button definito fuori dalla classe, ha quindi accesso al dato membro shape_
...
void Button::update(Flock& flock, sf::RenderWindow& window,
  ...
  if ((shape_.getGlobalBounds().contains(sf::Mouse::getPosition(window).x, //mouse get position di da un vettore con x e y (li prendi con .x e .y)
                                         sf::Mouse::getPosition(window).y)) &&
      click_state == clicked) {
    shape_.setFillColor(active_color_);
    switch (parameter_index_) { //parameter index e text sono parametri di button
      case sep:
        ...
        text.setString(roundto(flock.get_sep(), 2)); //arrotondi alla terza cifra
      ...
      case vis:
        ...
        text.setString(roundto(vision, 2)); //perchè non cè il pippozzo degli altri casi con globalbounds?
        break;
      default:
        break;
    }
...
void graphics(Flock& stormo) {
  ...
  double const dt{delta_t.asSeconds()}; //trasforma oggetto di tipo tempo in un double che indica i secondi passati
  int const fps{25}; //abbastanza standard
  int const steps_per_evolution{200 / fps}; //perchè 200 e non 1000 (1000 è troppo veloce)
  double dist_mult{1.};

  sf::RenderWindow window(sf::VideoMode(display_width, display_height),
                          "Flock Simulation", sf::Style::Default); //crea finestra con titolo e dimensioni
  ...
  std::vector<sf::Vector2f> boid_shape_vector{
      sf::Vector2f(5, 12), sf::Vector2f(1, 14),  sf::Vector2f(0, 10), //ogni vettore è un vertice
  ...
  float const boid_scale{display_width/(boid_scale_const * (boid_shape_vector[4].x - boid_shape_vector[2].x))}; //è la largheza del boid
  ...
  if (!background.loadFromFile("./boid_utilities/img/background.png")){ //se hai problemi a caricare, stampa
      std::cout << "Could not load background";
  }
  sf::Sprite background_sprite;
  background_sprite.setTexture(background); //serve per mettere lo sfondo con le giuste dimensioni rispetto alla finestra(si deforma insieme a essa)
  background_sprite.setScale(display_width / background_sprite.getLocalBounds().width, display_height / background_sprite.getLocalBounds().height); //la dilatazione è fatta in modo che lo sfondo copre sempre la finestra
  ...
  sf::IntRect rect_boom_sprite(0, 0, 448 / 8, 58); //left top width height la fai iniziare a sx e poi scorrere a destra (sono otto miniimmagini, quindi 7 passaggi)
  //i numeri non sono scelti a caso bensì rappresentano le dimensioni in pixel dell'immagine dell'esplosione presa (non è servito scalarla)
  sf::Sprite boom_sprite(boom_texture, rect_boom_sprite);//Construct the sprite from a sub-rectangle of a source texture
  boom_sprite.setOrigin(448 / 16, 29); //è il centro del rettangolo che gli viene passato (immagine nel rettangolo cambierà, ma l'origine della sprite è sempre il centro del rettangolo)
  ////// PARTE DEL MENÙ CON I PULSANTI

  // rettangolo del menu
  sf::Color menu_color(149, 149, 149, 255); //colore in rgb
  ...
  rect_init_setting(ali_text_rectangle, text_rectangle_width,
                    text_rectangle_height, 0.f, text_color, center_origin,
                    menu_rectangle_width * (33.f / 80.f),
                    display_height - menu_rectangle_height * (2.f / 5.f));//center origin è un eneumerator che vine gestito in un if della funzione
  ...
  sf::Text ali_text;
  text_init_setting(ali_text, font, 14, 0.f, roundto(stormo.get_al(), 2),
                    sf::Color::Black, menu_rectangle_width * (33.f / 80.f),
                    display_height - menu_rectangle_height * (5.f / 11.f));

  sf::Text coh_text;
  text_init_setting(coh_text, font, 14, 0.f, roundto(stormo.get_coh(), 2),
  ...
  sf::Clock animation_clock; //serve per indicare ogni quanto passare al riquadro successivo delle immagini (animazioni)
  ...
  while (window.isOpen()) { //non importano i frame per secondi, lui sa sempre tutti gli eventi accaduti
    ...
    if (click_state == clicked &&
        animation_clock.getElapsedTime().asSeconds() > 1.f &&//maggiore di 1 perchè è la durata dell esplosione, non puoi far andare un esplosione finhcè quella lanciata non finisce
        !(menu_rectangle.getGlobalBounds().contains(// non puoi lanciare esplosioni sopra al menù
            sf::Mouse::getPosition(window).x,
            sf::Mouse::getPosition(window).y)) &&
        !(stat_rectangle.getGlobalBounds().contains( //e neanche sul box delle statistiche
            sf::Mouse::getPosition(window).x,
            sf::Mouse::getPosition(window).y))) {
      sf::Vector2i mousePosition = sf::Mouse::getPosition(window); //Vector2i è come fare sf::Vector<int> (interi perchè coordinate in pixel)
      ...
      // dall'inizio
      animation_index = 0; //resetti a zero dopo un esplosione
      // boom_sound.play();
    }

    // dopo una certa quantità di tempo(1secondo) l'esplosione viene riportata fuori dallo
    ...
    if (animation_clock.getElapsedTime().asSeconds() > 1.f &&
      ...
      boom_positiony = 10000;
      //animation_clock.restart(); non mettere animation restart (altrimenti finita un esplosione devi aspettare 1 secondo)
    }
    ...
    rect_boom_sprite.left = animation_index * 58; //58 è la larghezza del rettangolo
    assert(rect_boom_sprite.left <= 406); //7*58 =392 (7 è il valore massimo di animation_index) (parte da 0 e arriva a 7(8 passaggi))
    boom_sprite.setTextureRect(rect_boom_sprite);//dopo aver cambiato la posizione del rettangolo esso viene assegnato alla sprite
    boom_sprite.setPosition(boom_positionx, boom_positiony);//  (invece che in alto a sinistra)
    ...
    update(stormo, steps_per_evolution, dt, dist_mult); //update diverso dalla funzione membro di Button

    // aggiornamento dei dati statistici
    if (statistics_clock.getElapsedTime().asSeconds() > 3.f) {//ogni 3 secondi
...
//ripulisci la finestra con una spiattellata di bianco (non serve se disegni il background)
    window.clear(sf::Color::White); //di default sarebbe un blu, ma non credo importi con o senza white perchè usi un immagine di sfondo
    ...
    click_state = unclicked;//serve perchè altrimenti il mouse una volta cliccato lo sarà sempre

    ++animation_index;
    if (animation_index == 8) {
      animation_index = 0;//fa ricominciare rect_boom_sprite all'inizio dell'immagine dell'esplosione
    }
  }
}
*/